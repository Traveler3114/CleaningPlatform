...
public class ClientDetailModel : PageModel
{
    ...
    public ClientDetailModel(ClientManager clientManager) { _clientManager = clientManager; }

    public ClientResponse? Client { get; set; }
    [BindProperty(SupportsGet = true)] public int Id { get; set; }
    [BindProperty] public UpdateClientProfileRequest Profile { get; set; } = new();
    [BindProperty] public string? ContactsJson { get; set; }
    [BindProperty] public UpsertSiteRequest NewSite { get; set; } = new();
    ...
    public async Task<IActionResult> OnGetAsync()
    {
        ...
        Profile = new UpdateClientProfileRequest
        {
            ...
            {
                Id = c.Id, ContactName = c.ContactName, Role = c.Role,
    ...
    {
        if (!User.HasPermission(PermissionKeys.ClientsEdit)) return Forbid();

        if (!string.IsNullOrWhiteSpace(ContactsJson))
        {
            var parsed = JsonSerializer.Deserialize<List<UpsertContactRequest>>(ContactsJson,
    ...
    {
        if (!User.HasPermission(PermissionKeys.ClientsDelete)) return Forbid();
