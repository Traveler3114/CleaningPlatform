...
public class InvoiceManager
{
    ...
    public async Task<Paginated<InvoiceResponse>> GetAllAsync(
        ...
        CancellationToken ct = default)
    {
        ...
        return new Paginated<InvoiceResponse>(items, totalCount);
    }
    ...
    public async Task<InvoiceResponse> GetByIdAsync(int id, CancellationToken ct = default)
    {
        ...
        if (invoice is null)
            throw new AppException("INVOICE_NOT_FOUND", $"Invoice #{id} was not found.", 404);
        return InvoiceMapper.ToResponse(invoice);
    }

    public async Task<InvoiceResponse> CreateFromBookingAsync(int bookingId, int? createdByEmployeeId, CancellationToken ct = default)
    {
        ...
        if (booking.Status != BookingStatus.Completed)
            throw new AppException("BOOKING_NOT_COMPLETED", $"Booking #{bookingId} cannot be invoiced — its current status is '{booking.Status}'. Only Completed bookings can generate an invoice.", 422);

        var existingLink = await _db.InvoiceBookings.AsNoTracking()
            .FirstOrDefaultAsync(ib => ib.BookingId == bookingId, ct);
        if (existingLink is not null)
            throw new AppException("BOOKING_ALREADY_INVOICED", $"Booking #{bookingId} is already linked to invoice #{existingLink.InvoiceId}. Open that invoice to record additional payments.", 409);
    ...
    public async Task<InvoiceResponse> RecordPaymentAsync(int invoiceId, RecordPaymentRequest request, int? recordedBy, CancellationToken ct = default)
    {
        if (request.Amount <= 0)
            throw new AppException("PAYMENT_AMOUNT_ZERO", _localizer["err_payment_amount_zero"], 422);
        ...
        if (!AllowedPaymentMethods.Contains(method))
            throw new AppException("INVALID_PAYMENT_METHOD", $"'{method}' is not a valid payment method. Accepted values: {string.Join(", ", AllowedPaymentMethods)}.", 422);

        var invoice = await _db.Invoices.Include(i => i.Payments).FirstOrDefaultAsync(i => i.Id == invoiceId, ct);
        if (invoice is null)
            throw new AppException("INVOICE_NOT_FOUND", $"Invoice #{invoiceId} was not found.", 404);
    ...
    public async Task<InvoiceResponse> UpdateStatusAsync(int invoiceId, string status, CancellationToken ct = default)
    {
        var trimmed = status?.Trim() ?? "";
        if (!AllowedStatuses.Contains(trimmed))
            throw new AppException("INVALID_INVOICE_STATUS", $"'{trimmed}' is not a valid invoice status. Accepted values: {string.Join(", ", AllowedStatuses)}.", 422);
