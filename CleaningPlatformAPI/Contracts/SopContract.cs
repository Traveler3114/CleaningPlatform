...
public record BookingSopAssignmentResponse(int BookingId, int SopTemplateId, string SopName, string? CustomInstructions, int TotalItems, int CompletedItems, List<ChecklistResponseResponse> ChecklistItems);
public record ChecklistResponseResponse(int ChecklistItemId, string ItemText, bool IsRequired, bool IsCompleted, DateTime? CompletedAt, string? Notes);
