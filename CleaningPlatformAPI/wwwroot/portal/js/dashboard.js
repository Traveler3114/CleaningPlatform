// dashboard.js
var _cacheDashboardData = null;

function renderKpis(data) {
    document.getElementById('kpi-grid').innerHTML =
        '<div class="kpi-card"><span>' + __('section_upcoming_bookings') + '</span><strong>' + data.upcomingBookings + '</strong></div>' +
        '<div class="kpi-card"><span>' + __('status_completed') + '</span><strong>' + data.completedBookings + '</strong></div>' +
        '<div class="kpi-card"><span>' + __('kpi_total_revenue') + '</span><strong>' + formatCurrency(data.totalSpent) + '</strong></div>' +
        '<div class="kpi-card"><span>' + __('kpi_outstanding') + '</span><strong>' + formatCurrency(data.outstandingAmount) + '</strong></div>';
}

function renderUpcomingBookings(bookings) {
    var container = document.getElementById('upcoming-bookings');
    if (bookings.length === 0) {
        container.innerHTML = '<div class="empty-state"><p>' + __('empty_no_upcoming') + '</p></div>';
        return;
    }
    var html = bookings.map(function (b) {
        var svcNames = (b.services || []).map(function (s) { return s.serviceName; }).join(', ');
        var estTotal = (b.services || []).reduce(function (sum, s) { return sum + (s.estimatedPrice || s.finalPrice || 0); }, 0);
        return '<div class="booking-card booking-card--' + b.status.toLowerCase() + '" onclick="window.location.href=\'booking-detail.html?id=' + b.id + '\'">' +
            '<div class="booking-card__header">' +
            '<div class="booking-card__title">' + b.serviceType + ' &mdash; ' + formatDate(b.date) + '</div>' +
            statusBadge(b.status) +
            '</div>' +
            '<div class="booking-card__meta">' + formatTime(b.hour) + ' &middot; ' + svcNames + '</div>' +
            '<div class="booking-card__footer">' +
            '<span>' + (b.siteName || __('empty_no_sites')) + '</span>' +
            '<span>' + formatCurrency(estTotal) + '</span>' +
            '</div>' +
            '</div>';
    }).join('');
    container.innerHTML = html;
}

function renderRecentInvoices(invoices) {
    var container = document.getElementById('recent-invoices');
    if (invoices.length === 0) {
        container.innerHTML = '<div class="empty-state"><p>' + __('empty_no_invoices_yet') + '</p></div>';
        return;
    }
    var html = '<table class="portal-table"><thead><tr><th>' + __('nav_invoices') + '</th><th>' + __('th_date') + '</th><th>' + __('th_status') + '</th><th>' + __('th_amount') + '</th></tr></thead><tbody>';
    invoices.slice(0, 5).forEach(function (i) {
        html += '<tr onclick="window.location.href=\'invoice-detail.html?id=' + i.id + '\'">' +
            '<td><a href="invoice-detail.html?id=' + i.id + '" class="link">' + i.invoiceNumber + '</a></td>' +
            '<td>' + formatDate(i.issueDate) + '</td>' +
            '<td>' + statusBadge(i.status) + '</td>' +
            '<td>' + formatCurrency(i.totalAmount) + '</td>' +
            '</tr>';
    });
    html += '</tbody></table>';
    container.innerHTML = html;
}

function loadPortalDashboard() {
    apiFetch('/api/portal/dashboard').then(function (data) {
        _cacheDashboardData = data;
        renderKpis(data);
        renderUpcomingBookings(data.upcomingBookingList);
        renderRecentInvoices(data.recentInvoices);
    }).catch(function (err) {
        showError(err.message || __('msg_failed_dashboard'));
    });
}

loadPortalDashboard();
window.addEventListener('i18nReady', function () {
    if (_cacheDashboardData) {
        renderKpis(_cacheDashboardData);
        renderUpcomingBookings(_cacheDashboardData.upcomingBookingList);
        renderRecentInvoices(_cacheDashboardData.recentInvoices);
    } else {
        loadPortalDashboard();
    }
});
